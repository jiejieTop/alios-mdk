 ...
 * 
 */
aos_timer_t Timer1_Handle;
aos_timer_t Timer2_Handle;
...
static void AppTaskCreate(void* arg)
{
  ...
  cpsr = cpu_intrpt_save();
	...
	res = aos_timer_new(&Timer1_Handle,Timer1_Callback,0,1000,1);
 
	if(0 == res)
    printf("创建定时器1成功!\r\n");
	
	res = aos_timer_new(&Timer1_Handle,Timer2_Callback,0,5000,0);
